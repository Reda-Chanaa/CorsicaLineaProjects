...
def Stat_TUN(data_yesterday, data_today, annee, mois, cible, budget):
    ...
    mesure = pd.DataFrame()
    mesure["TUNISIE"] = df_mask_cumul['MOIS'].apply(
...
def Stat_TUN_Objectif(data_yesterday, data_today, annee, mois, cible, budget,objectif):
    table_reseau_armateur_today = pd.pivot_table(
        data_today[(data_today.RESEAU == "TUNISIE")
                   & (data_today.ARMATEUR == "CL")],
        index=['ANNEE', 'MOIS'],
    ...
    BS1 = pd.DataFrame(columns=['TUNISIE', 'CIBLE', 'VENTE', "CUMUL", "BUDGET","OBJECTIF"])
    MS1 = pd.DataFrame(columns=['TUNISIE', 'CIBLE', 'VENTE', "CUMUL", "BUDGET","OBJECTIF"])
    HS = pd.DataFrame(columns=['TUNISIE', 'CIBLE', 'VENTE', "CUMUL", "BUDGET","OBJECTIF"])
    MS2 = pd.DataFrame(columns=['TUNISIE', 'CIBLE', 'VENTE', "CUMUL", "BUDGET","OBJECTIF"])
    BS2 = pd.DataFrame(columns=['TUNISIE', 'CIBLE', 'VENTE', "CUMUL", "BUDGET","OBJECTIF"])
    print("-------------------------------")
    print(mesure)
    bud = 0
    for i in mesure.index:
...
def Stat_TUN_plus(data_yesterday, data_today, annee, mois):
    ...
    BS1 = pd.DataFrame(
        columns=['TUNISIE', 'CIBLE/JR', 'VENTE', "CUMUL", "BUDGET"])
    MS1 = pd.DataFrame(
        columns=['TUNISIE', 'CIBLE/JR', 'VENTE', "CUMUL", "BUDGET"])
    HS = pd.DataFrame(
        columns=['TUNISIE', 'CIBLE/JR', 'VENTE', "CUMUL", "BUDGET"])
...
def Stat_TUN(data_yesterday, data_today, annee, mois):
    table_reseau_armateur_today = pd.pivot_table(
        data_today[(data_today.RESEAU == "TUNISIE")
                   & (data_today.ARMATEUR == "CL")],
        index=['ANNEE', 'MOIS'],
...
def StatTUNObj(request):
    if request.method == 'POST':
        ...
        # Stats
        data = Stat_TUN_Objectif(df1, df2, annee, MOIS, CIBLE, BUDGET,OBJECTIF)
        print('data : ', data)
...
def StatTUN(request):
    if request.method == 'POST':
        ...
        BUDGET = list(map(int, BUDGET))
        # Stats
        data = Stat_TUN(df1, df2, annee, MOIS, CIBLE, BUDGET)
        print('data : ', data)
...
def StatTUNPLUS(request):
    if request.method == 'POST':
        ...
        MOIS = list(map(int, MOIS))
        # Stats
        data = Stat_TUN_plus(df1, df2, annee, MOIS)
        print('data : ', data)
    return HttpResponse(data.to_json(orient='records'))
