...
def MesureALG(request):
    ...
    if request.method == 'POST':
        ...
        # mesure 1
        df_sauv=pd.pivot_table(df_sauv[(df_sauv.RESEAU == "ALGERIE") &(df_sauv.ARMATEUR == "CL")], index=['SAIL_ID','NAVIRE','MOIS','DATE_DEPART','PORT_DEPART','PORT_ARRIVEE'],aggfunc={'PAX':np.sum})
        ...
        # mesure 2
        df_JOUR=pd.pivot_table(df_JOUR[(df_JOUR.RESEAU == "ALGERIE") &(df_JOUR.ARMATEUR == "CL")], index=['SAIL_ID','NAVIRE','MOIS','DATE_DEPART','PORT_DEPART','PORT_ARRIVEE'],aggfunc={'PAX':np.sum})
