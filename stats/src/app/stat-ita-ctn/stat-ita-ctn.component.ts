...
export class ApiStat {
  ...
  sendFile(File1: any, File2: any, annee: any, mois: any): Observable<any> {
    ...
    return this.http.post(this.baseurl + '/stat-ita-ctn/', formData, { headers: header })
  }
}
...
export class StatItaCtnComponent{
  ...
  displayedColumns: string[] = ['ITALIE', 'VENTEYES', 'VENTE', 'CUMULE', 'ECART'];
  ...
  executeVisualisation() {
    let c0: Observable<boolean> = this.ITALIE.valueChanges;
    ...
    merge(c0, c1, c2, c3, c4).subscribe(v => {
      this.columnDefinitions[0].show = this.ITALIE.value;
  ...
  InitializeVisualization() {
    this.columnDefinitions = [
      { def: 'ITALIE', label: 'ITALIE', show: this.ITALIE.value },
