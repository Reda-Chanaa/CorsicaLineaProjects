...
export class ApiStat {
  ...
  getInf(annee: any): Observable<any> {
    ...
    return this.http.post(this.baseurl + '/stat-alg-info/', formData, { headers: header })
  }
  ...
  sendFileObj(File1: any, File2: any, annee: any, mois: any, cible: any, budget: any, objectif: any): Observable<any> {
    ...
    var header = new HttpHeaders();
    header.append('Content-Type', 'multipart/form-data');
    return this.http.post(this.baseurl + '/stat-alg-obj/', formData, { headers: header })
  }
  sendFilePlus(File1: any, File2: any, annee: any, mois: any): Observable<any> {
    ...
    var header = new HttpHeaders();
    header.append('Content-Type', 'multipart/form-data');
    return this.http.post(this.baseurl + '/stat-alg-plus/', formData, { headers: header })
  }
}
...
export class StatAlgComponent {
  ...
  displayedColumns: string[] = ['ALGERIE', 'CIBLE', 'VENTE', 'CUMULE', 'BUDGET', 'OBJECTIF'];
  ...
  getchart(data) {
    ...
    for (let index = 0; index < data.length; index++) {
      if (data[index].ALGERIE == "Janvier") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible1)
        mois.push("Janvier")
      }
      if (data[index].ALGERIE == "Février") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible2),
          mois.push("Février")
      }
      if (data[index].ALGERIE == "Mars") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible3)
        mois.push("Mars")
      }
      if (data[index].ALGERIE == "Avril") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible4)
        mois.push("Avril")
      }
      if (data[index].ALGERIE == "Mai") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible5)
        mois.push("Mai")
      }
      if (data[index].ALGERIE == "Juin") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible6)
        mois.push("Juin")
      }
      if (data[index].ALGERIE == "Juillet") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible7)
        mois.push("Juillet")
      }
      if (data[index].ALGERIE == "Août") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible8)
        mois.push("Août")
      }
      if (data[index].ALGERIE == "Septembre") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible9)
        mois.push("Septembre")
      }
      if (data[index].ALGERIE == "Octobre") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible10)
        mois.push("Octobre")
      }
      if (data[index].ALGERIE == "Novembre") {
        vente.push(Number(data[index].VENTE))
        cible.push(this.cible11)
        mois.push("Novembre")
      }
      if (data[index].ALGERIE == "Décembre") {
        vente.push(Number(data[index].VENTE))
    ...
    Highcharts.chart('container', {
      title: {
          text: 'Ventes journalières par rappot à la cible ( ALGERIE )'
      },
      xAxis: {
          categories: mois
      },
  ...
  exportTable() {
    ...
    let date = "ReportingALG_" + this.annee + "_" + day + "-" + mois + "-" + annee + ".xlsx"
    console.log(date)
    // save to file 
